...
Cursos da meta sem publico mapeado recebem o rotulo "(sem publico)" para que
o total por publico reconcilie com o total geral (curso 724, p.ex.).
"""
...
def main() -> int:
    ...
    # Cursos da meta sem nenhum publico -> bucket "(sem publico)".
    nao_mapeados = [cid for cid in alvo if cid not in mapeados]
    for cid in nao_mapeados:
        rows.append({
            "id_curso": cid,
            "nome_curso": alvo[cid],
            "publico_alvo": SEM_PUBLICO,
            "programa_trilha": SEM_TRILHA,
        })
    ...
    print(f"  · {len(mapeados)} cursos mapeados + {len(nao_mapeados)} sem publico")
